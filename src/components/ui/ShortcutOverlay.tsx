...
export default function ShortcutOverlay({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        ref={overlayRef}
        className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl shadow-elevated p-6 w-full max-w-sm animate-slide-down"
                ...
                {items.map(({ keys, desc }) => (
                  <div key={desc} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      {keys.map((key, i) => (
                        <span key={i}>
                          {i > 0 && <span className="text-[var(--text-muted)] text-xs mx-0.5">+</span>}
                          <kbd className="inline-flex items-center justify-center min-w-[24px] h-6 px-1.5 rounded border border-[var(--border)] bg-[var(--bg-elevated)] text-xs font-mono text-[var(--text-secondary)]">
                            {key}
                          </kbd>
                        </span>
                      ))}
